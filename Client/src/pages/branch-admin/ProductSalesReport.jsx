...
export default function ProductSalesReport() {
  ...
  const availableColumns = [{
    ...
    key: "prod_name",
    label: t("reports.product_name", "Product Name")
  }, {
    key: "quantity",
    label: t("reports.quantity_sold", "Quantity Sold")
  }, {
    key: "unit_price",
    label: t("reports.unit_price", "Unit Price")
  }, {
    key: "total_sale",
    label: t("reports.total_sale", "Total Sale")
  }];
const {
    user
  } = useAuth();
...
const formattedRows = rows.map(row => {
  ...
  const exportPDFHtml = () => {
    ...
    printWindow.document.write('<!DOCTYPE html>' +
      ...
      '<head>' +
      '<meta charset="UTF-8" />' +
      '<title>' + t('branch_admin.product_sales_report', 'Product Sales Report') + '<\/title>' +
      ...
      '</head>' +
      '<body>' +
      '<h2>' + t('branch_admin.product_sales_report', 'Product Sales Report') + '</h2>' +
  ...
  return <div className="w-full min-h-screen flex bg-slate-50 text-slate-800 antialiased overflow-visible">
      ...
      marginLeft: 240
        ...
        <h1 className="text-2xl px-5 py-2 font-bold tracking-tight text-gray-600 ">{t("branch_admin.products_sales_report", "Products Sales Report")}</h1>
