...
const {
  ...
  const cashierStats = useMemo(() => {
    ...
    rangeOrders.forEach(order => {
      ...
      const entry = totals.get(cashierId) || {
        revenue: 0,
        orders: 0
      };
    ...
    return cashierUsers.map(cashier => {
      const metrics = totals.get(cashier.u_id) || {
        revenue: 0,
        orders: 0
      };
      ...
      minHeight: "100vh"
					...
					<div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mb-6">
						...
						<div className="flex items-center gap-3">
							...
							<button type="button" className="flex items-center gap-2 rounded-full bg-[#0D5EA8] px-4 py-2 text-xs font-semibold text-white shadow">
								<FaDownload />{t("branch_admin.export_report", "Export Report")}</button>
						</div>
					</div>
            ...
            backgroundColor: "#B7F5BC"
							...
							<div className="w-10 h-10 rounded-full  flex items-center justify-center">
								<img src={topPerformerIcon} alt="Top performer" className="h-10 w-10 object-contain" />
							</div>
							<div>
            ...
            backgroundColor: "#FFC0D4"
							...
							<div className="w-10 h-10 rounded-full flex items-center justify-center">
								<img src={timeIcon} alt="Average processing time" className="h-10 w-10 object-contain" />
							</div>
							<div>
            ...
            backgroundColor: "#A8E6FF"
							...
							<div className="w-10 h-10 rounded-full  flex items-center justify-center">
								<img src={salesIcon} alt="Total branch sales" className="h-10 w-10 object-contain" />
							</div>
							<div>
					...
					<div className="bg-white rounded-2xl border border-slate-100 p-6 shadow-sm mb-6">
						<div className="flex items-center justify-between">
							<div>
								...
								<p className="text-xs text-slate-400">{t("branch_admin.comparison_of_total_sales_generated_per_", "Comparison of total sales generated per staff member")}</p>
							</div>
							<div className="flex items-center gap-2 text-xs text-slate-500">
								<span className="h-2 w-2 rounded-full bg-[#0D5EA8]" />{t("branch_admin.primary_revenue", "Primary Revenue")}</div>
						</div>
						<div className="h-56 mt-4">
							{isLoading ? <div className="h-full rounded-xl bg-slate-50 animate-pulse" /> : <Bar data={revenueChartData} options={barOptions} />}
						</div>
					</div>
					...
					<div className="bg-white rounded-2xl border border-slate-100 p-6 shadow-sm">
						...
						<div className="overflow-x-auto">
							<table className="min-w-full text-xs">
								...
								<tbody>
									{sortedCashiers.length === 0 && !isLoading && <tr>
											<td colSpan="4" className="py-4 text-slate-500">{t("branch_admin.no_cashier_data_available", "No cashier data available.")}</td>
										</tr>}
									{(isLoading ? Array.from({
                  length: 4
                }) : sortedCashiers.slice(0, 4)).map((cashier, index) => {
                  if (!cashier) {
                    return <tr key={`cashier-row-${index}`} className="border-b">
                  ...
                  const initials = cashier.name.split(" ").map(part => part[0]).join("").slice(0, 2).toUpperCase();
                  return <tr key={cashier.id} className="border-b last:border-b-0">
												<td className="py-3">
													<div className="flex items-center gap-3">
						...
						<div className="mt-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 text-[11px] text-slate-400">
							<div>{t("branch_admin.showing_4_of", "Showing 4 of")}{cashierUsers.length || 0}{t("branch_admin.cashiers_registered", "cashiers registered")}</div>
							<div className="flex items-center gap-2">
								<button type="button" className="px-3 py-1 rounded-lg border border-slate-200 bg-white text-slate-500">{t("branch_admin.previous", "Previous")}</button>
								...
								<button type="button" className="px-3 py-1 rounded-lg border border-slate-200 bg-white text-slate-500">{t("branch_admin.next", "Next")}</button>
							</div>
						</div>
					</div>
				</div>
