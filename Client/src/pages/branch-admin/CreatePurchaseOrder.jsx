...
const CreatePurchaseOrder = () => {
  ...
  const availableItems = useMemo(() => {
    ...
    const rm = rawMaterials
      .filter((m) => !term || m.rm_name.toLowerCase().includes(term))
      .map((m) => ({ key: `rm_${m.rm_id}`, rm_id: m.rm_id, name: m.rm_name, unit: m.unit }));
    const pro = branchProducts
      .filter((p) => !term || p.pro_name.toLowerCase().includes(term))
      .map((p) => ({ key: `pro_${p.pro_id}`, pro_id: p.pro_id, name: p.pro_name, unit: "pcs" }));
    return [...rm, ...pro];
  ...
  const addToCart = (item) => {
    ...
    setCart((prev) => [
      ...prev,
      { ...item, qty: "", sup_id: defaultSupplier || "" },
    ]);
  };
  ...
  const applyDefaultSupplierToAll = (sup_id) => {
    ...
    setCart((prev) => prev.map((c) => ({ ...c, sup_id })));
  };
...
if (!branchId) {
    ...
    try {
      ...
      for (const supId of supplierIds) {
        ...
        for (const item of items) {
          const itemRes = await fetch("/api/purchase-items", {
            ...
            body: JSON.stringify({
              po_id: order.po_id,
              ...(item.rm_id ? { rm_id: item.rm_id } : { pro_id: item.pro_id }),
      ...
      showToast(
        `${ordersCreated} purchase order${ordersCreated > 1 ? "s" : ""} placed successfully`
      );
      ...
      <div className="flex flex-col h-screen overflow-hidden" style={{ marginLeft: 240 }}>
        ...
        <main className="flex-1 overflow-y-auto p-4 lg:p-8">
          <div className="max-w-6xl mx-auto space-y-6">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900">Bulk Order</h1>
              <p className="text-sm text-gray-500 mt-1">
                Add multiple items to order in one go
              </p>
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
              <label className="block text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-2">
                Default Supplier (applies to items you add below)
              </label>
              <select
                className="w-full max-w-sm border-gray-200 border rounded-xl p-3 text-sm outline-none bg-white"
                value={defaultSupplier}
                onChange={(e) => applyDefaultSupplierToAll(e.target.value)}
              >
                <option value="">Choose a default supplier</option>
                {suppliers.map((s) => (
                  <option key={s.sup_id} value={s.sup_id}>{s.sup_name}</option>
                ))}
              </select>
              <p className="mt-2 text-xs text-gray-400">
                You can still override the supplier per item below if some items come from a different supplier.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {/* Item picker */}
              <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                <h2 className="text-sm font-bold text-gray-800 mb-4">Select Items</h2>
                <input
                  type="text"
                  placeholder="Search ingredients or products..."
                  value={searchTerm}
                  onChange={(e) => setSearchTerm(e.target.value)}
                  className="w-full border-gray-200 border rounded-xl p-3 text-sm outline-none mb-4"
                />
                  ...
                  {availableItems.map((item) => {
                    ...
                    return (
                      <button
                        ...
                        disabled={inCart}
                        className={`w-full text-left px-4 py-2.5 rounded-xl border flex justify-between items-center transition ${
                          inCart
              ...
              {/* Cart */}
              <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                <h2 className="text-sm font-bold text-gray-800 mb-4">Order List ({cart.length})</h2>
                {cart.length === 0 ? (
                      ...
                      <div key={item.key} className="border border-gray-200 rounded-xl p-3 space-y-2">
                        ...
                        <div className="flex gap-2">
                          <input
                            ...
                            className="flex-1 border-gray-200 border rounded-lg p-2 text-sm outline-none"
                          />
                          <select
                            value={item.sup_id}
                            onChange={(e) => updateCartItem(item.key, "sup_id", e.target.value)}
                            className="flex-1 border-gray-200 border rounded-lg p-2 text-sm outline-none bg-white"
                          >
            ...
            {cart.length > 0 && Object.keys(groupedBySupplier).length > 1 && (
              <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 text-sm text-blue-700">
                This will create <strong>{Object.keys(groupedBySupplier).length} separate orders</strong>, one per supplier:{" "}
            ...
            <div className="flex justify-end gap-3">
              <button
                ...
                className="px-5 py-2.5 border border-gray-200 text-gray-600 rounded-xl font-medium hover:bg-gray-50"
              >
                Cancel
              </button>
              <button
                ...
                className="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-medium disabled:opacity-60"
              >
