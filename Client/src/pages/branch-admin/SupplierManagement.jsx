...
console.log('Socket disconnected in SupplierManagement');
    ...
    const handleSupplierCreated = newSupplier => {
      ...
      setSuppliers(prevSuppliers => {
        ...
        const exists = prevSuppliers.some(s => s.sup_id === newSupplier.sup_id || s.sup_email?.toLowerCase() === newSupplier.sup_email?.toLowerCase());
        if (exists) {
    ...
    const handleSupplierUpdated = updatedSupplier => {
      ...
      setSuppliers(prevSuppliers => {
        const updated = prevSuppliers.map(s => s.sup_id === updatedSupplier.sup_id ? {
          ...s,
          ...updatedSupplier
        } : s);
        localStorage.setItem('cached_suppliers', JSON.stringify(updated));
  ...
  return <div className="flex bg-gray-50 min-h-screen">
      ...
      <div className="flex-1 ml-[240px]">
        <Header title={t("branch_admin.suppliers", "Suppliers")} role="Branch Admin" />
        ...
        <div className="p-8 max-w-[1200px] mx-auto">
          {selectedSupplier ? <SupplierDetailView supplier={selectedSupplier} onBack={() => setSelectedSupplier(null)} showToast={showToast} /> : <>
              <div className="mb-6">
                <h1 className="text-2xl font-bold text-gray-900 m-0">{t("branch_admin.supplier_directory", "Supplier Directory")}</h1>
                <p className="text-gray-500 mt-1.5 text-sm">{t("branch_admin.suppliers_assigned_to_your_branch_contac", "Suppliers assigned to your branch. Contact your Company Admin to add new suppliers.")}</p>
              </div>

              {error && <div className="bg-red-50 border border-red-300 rounded-xl px-4 py-3 text-red-600 mb-5 font-medium">
                  {error}
                </div>}

              {isLoading ? <p className="text-gray-500">{t("branch_admin.loading_suppliers", "Loading suppliers...")}</p> : suppliers.length === 0 ? <div className="text-center py-20 text-gray-400">
                  <div className="text-5xl mb-3">🏢</div>
                  <p className="font-semibold text-base mb-1">{t("branch_admin.no_suppliers_assigned_to_this_branch", "No suppliers assigned to this branch")}</p>
                  <p className="text-sm">{t("branch_admin.ask_your_company_admin_to_add_suppliers_", "Ask your Company Admin to add suppliers to this branch.")}</p>
                </div> : <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
                  {suppliers.map(sup => <div key={sup.sup_id} onClick={() => setSelectedSupplier(sup)} className="bg-white p-6 rounded-2xl border border-gray-200 cursor-pointer shadow-sm hover:shadow-lg transition-all duration-200 transform hover:-translate-y-1">
