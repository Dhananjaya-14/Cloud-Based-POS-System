      ...
      marginLeft: 240
        ...
        <div className="p-8 bg-gray-50 min-h-screen">
          ...
          <div className="flex gap-6 mb-8">
            <StatCard title={t("branch_admin.items_out_of_stock", "Items Out of Stock")} count={isLoading ? '...' : stats.outOfStock} subtitle={t("branch_admin.immediate_kitchen_impact", "Immediate kitchen impact. Essential items are depleted.")} badgeText={t("branch_admin.critical", "CRITICAL")} badgeColor="bg-red-500" bgColor={activeFilter === 'out' ? 'bg-red-100 ring-2 ring-red-400' : 'bg-red-50'} textColor="text-red-700" icon="⭕" actionText={t("branch_admin.reorder_immediately", "Reorder immediately")} onClick={() => setActiveFilter(activeFilter === 'out' ? 'all' : 'out')} />
            <StatCard title={t("branch_admin.items_low_stock", "Items Low Stock")} count={isLoading ? '...' : stats.lowStock} subtitle={t("branch_admin.replenish_soon", "Replenish soon to avoid service disruption. Stocks under threshold.")} badgeText={t("branch_admin.warning", "WARNING")} badgeColor="bg-blue-600" bgColor={activeFilter === 'low' ? 'bg-blue-100 ring-2 ring-blue-400' : 'bg-blue-50'} textColor="text-blue-700" icon="⚠️" actionText={t("branch_admin.view_details", "View Details")} onClick={() => setActiveFilter(activeFilter === 'low' ? 'all' : 'low')} />
          </div>
