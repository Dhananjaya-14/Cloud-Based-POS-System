...
const availableColumns = [
  { key: "rm_name", label: "Raw Material Name" },
  { key: "unit", label: "Unit" },
  ...
  const exportExcel = () => {
    const formattedRows = rows.map((row) => {
      ...
      if (selectedColumns.includes("stock_qty")) {
        excelRow["Stock Quantity"] = Number(
          row.stock_qty || 0
        ).toFixed(2);
      }
    ...
    const worksheet =
      XLSX.utils.json_to_sheet(formattedRows);
    ...
    formattedRows.forEach((row) => {
      Object.keys(row).forEach((key, colIndex) => {
        ...
        maxColumnWidths[colIndex] = Math.max(
          maxColumnWidths[colIndex] || 10,
          currentLength + 5
        );
      });
    });
    ...
    XLSX.writeFile(
      workbook,
      `Raw_Material_Stock_Report_${timestamp}.xlsx`
    );
  };
    ...
    <div className="w-full min-h-screen flex bg-slate-50 text-slate-800 antialiased overflow-visible">
      ...
      >
        ...
        <button
          ...
          className="fixed bottom-6 right-6 inline-flex items-center gap-2 bg-slate-900 hover:bg-slate-800 text-white text-sm font-semibold px-4 py-2.5 rounded-lg shadow-lg hover:shadow-xl active:scale-95 transition-all duration-150 z-50 group border border-slate-700"
        >
