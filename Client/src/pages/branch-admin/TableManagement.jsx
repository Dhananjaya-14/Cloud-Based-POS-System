...
const Card = ({ children, className = '', hover = false, ...props }) => {
  return (
    <div 
      className={`bg-white rounded-xl border border-gray-200 shadow-sm ${hover ? 'transition-all hover:-translate-y-1 hover:shadow-lg' : ''} ${className}`} 
      {...props}
    >
      {children}
    </div>
...
const Input = ({ label, id, className = '', ...props }) => {
  return (
    <div className={`w-full ${className}`}>
      ...
      <input
        id={id}
        className="w-full px-4 py-3 text-sm text-gray-900 bg-gray-50 border border-gray-200 rounded-lg outline-none focus:bg-white focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition-colors"
        {...props}
      />
    </div>
  );
...
const Select = ({ label, id, options, className = '', ...props }) => {
  return (
    <div className={`w-full ${className}`}>
      ...
      <select
        id={id}
        className="w-full px-4 py-3 text-sm text-gray-900 bg-gray-50 border border-gray-200 rounded-lg outline-none focus:bg-white focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition-colors cursor-pointer"
        {...props}
      >
        {options.map((opt, i) => (
...
const Badge = ({ children, variant = 'gray', className = '', ...props }) => {
  ...
  return (
    <span 
      className={`inline-flex items-center gap-1.5 px-3 py-1 text-xs font-bold uppercase tracking-wide border rounded-full ${variants[variant] || variants.gray} ${className}`}
      {...props}
    >
      {children}
    </span>
...
const TableManagement = () => {
  ...
  const [formData, setFormData] = useState({ table_number: "", chair_count: 2, status: "Available", area: "Main Hall" });
  ...
  const openAddModal = () => {
    setEditTarget(null);
    setFormData({ table_number: "", chair_count: 2, status: "Available", area: "Main Hall" });
    setIsModalOpen(true);
  };

  const openEditModal = (table) => {
    setEditTarget(table);
    setFormData({ table_number: table.table_number, chair_count: table.chair_count, status: table.status, area: table.area });
    setIsModalOpen(true);
  };
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      case "Available": return <Badge variant="green"><FaCheck /> {status}</Badge>;
      case "Occupied": return <Badge variant="red"><FaHeartbeat /> {status}</Badge>;
      case "Reserved": return <Badge variant="amber"><FaClock /> {status}</Badge>;
  ...
  return (
    <div className="flex min-h-screen bg-gray-50">
        ...
        <div className="flex-1 p-6 md:p-8 overflow-y-auto font-sans text-gray-900 bg-gray-50">
          ...
          <div className="flex items-start justify-between mb-8">
            <div>
              ...
              <p className="mt-1 text-xs font-semibold tracking-wider text-gray-500 uppercase">
                Manage Floor Capacity and Live Status
              </p>
            </div>
          ...
          <div className="grid grid-cols-2 gap-5 mb-6 md:grid-cols-4">
            <Card className="flex items-center gap-4 p-5">
              ...
              <div>
                ...
                <p className="m-0 mt-0.5 text-xs font-semibold tracking-wide text-gray-500 uppercase">Total Tables</p>
              </div>
            </Card>
            ...
            <Card className="flex items-center gap-4 p-5">
              ...
              <div>
                ...
                <p className="m-0 mt-0.5 text-xs font-semibold tracking-wide text-gray-500 uppercase">Available</p>
              </div>
            </Card>
            ...
            <Card className="flex items-center gap-4 p-5">
              ...
              <div>
                ...
                <p className="m-0 mt-0.5 text-xs font-semibold tracking-wide text-gray-500 uppercase">Occupied</p>
              </div>
            </Card>
            ...
            <Card className="flex items-center gap-4 p-5">
              ...
              <div>
                ...
                <p className="m-0 mt-0.5 text-xs font-semibold tracking-wide text-gray-500 uppercase">Reserved</p>
              </div>
            </Card>
          </div>
          ...
          <Card className="flex flex-wrap items-center gap-4 p-4 mb-6">
            <div className="flex items-center flex-1 min-w-[200px] px-3 bg-gray-50 border border-gray-200 rounded-lg">
              ...
              <input 
                ...
                className="w-full p-3 text-sm text-gray-900 bg-transparent border-none outline-none"
              />
            </div>
            ...
            <Select 
              ...
              options={["All Areas", "Main Hall", "Bar Counter", "Terrace", "Garden", "VIP Deck", "Private Room"]} 
            />
            ...
            <Select 
              ...
              className="!w-auto min-w-[140px]"
              options={["All Status", "Available", "Occupied", "Reserved"]} 
            />
            ...
            <Select 
              ...
              className="!w-auto min-w-[140px]"
              options={["Any Capacity", "2 Persons", "4 Persons", "6+ Persons"]} 
            />

            <div className="flex gap-2 ml-auto">
              <Button 
                variant={viewMode === "list" ? "activeGhost" : "secondary"} 
              ...
              <Button 
                variant={viewMode === "grid" ? "activeGhost" : "secondary"} 
          ...
          {loading ? (
             <div className="flex flex-col items-center justify-center p-20 text-gray-400">
               ...
               <p className="font-semibold text-gray-500">Loading Tables...</p>
             </div>
          ) : viewMode === "grid" ? (
            <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
              {filteredTables.map(table => (
                <Card key={table.id} hover={true} className="relative p-5 cursor-default">
                  <div className="flex items-start justify-between mb-6">
                    <div>
                      <h2 className="m-0 text-2xl font-bold text-gray-900">{table.table_number}</h2>
                      <p className="m-0 text-[10px] font-bold tracking-widest text-gray-500 uppercase">Table</p>
                    </div>
                  ...
                  <div className="flex items-center justify-between mb-3 text-xs text-gray-900">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="font-semibold">Capacity: {table.chair_count}</span>
                    </div>
              ...
              <TableBody>
                {filteredTables.map(table => (
                  <TableRow key={table.id}>
                    ...
                    <TableCell className="font-medium text-center text-gray-500">{table.chair_count} Persons</TableCell>
          ...
          {filteredTables.length === 0 && (
             <div className="p-16 text-center text-gray-400">
               ...
               <h3 className="text-lg font-semibold text-gray-500">No tables found matching your criteria.</h3>
             </div>
          )}
        </div>
      </div>
        ...
        <form onSubmit={handleSave} className="space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            <Input 
              ...
              onChange={e => setFormData({ ...formData, chair_count: e.target.value })}
            />
            <Select 
              id="status"
              label="Status"
      ...
      <Modal isOpen={deleteModalOpen} onClose={() => { setDeleteModalOpen(false); setTableToDelete(null); }}>
        <div className="text-center pb-2">
          <div className="flex items-center justify-center w-16 h-16 mx-auto mb-5 rounded-full bg-red-100 text-red-500">
            <FaExclamationTriangle size={32} />
          </div>
          <h2 className="mb-3 text-xl font-bold text-gray-900">Delete Table?</h2>
          <p className="mb-8 text-sm text-gray-500">
            Are you sure you want to delete this table?
          </p>
          <div className="flex gap-3">
            <Button variant="secondary" onClick={() => { setDeleteModalOpen(false); setTableToDelete(null); }} className="flex-1">
              Cancel
            </Button>
