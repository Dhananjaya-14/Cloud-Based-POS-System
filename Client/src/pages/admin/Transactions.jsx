  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        if (filters.branch !== "all") orderParams.b_id = filters.branch;
        ...
        const normalizedPayments = (supplierPayments || []).map((p) => {
          const po = purchaseOrdersById[p.po_id];
          const branchName =
            po?.B_name ?? po?.b_name ?? branchById[po?.b_id ?? p.b_id]?.B_name ?? p.B_name ?? p.b_name ?? null;
