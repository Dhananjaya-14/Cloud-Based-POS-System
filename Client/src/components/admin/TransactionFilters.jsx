        ...
        export default function TransactionFilters({ filters, setFilters }) {
          ...
          useEffect(() => {
            ...
            (async () => {
              try {
                ...
                if (isMounted) setBranches(response || []);
              } catch {
                if (isMounted) setBranches([]);
              }
            })();
          ...
          return (
            <div className="space-y-4 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
              ...
              <div className="grid grid-cols-1 gap-4 border-t border-slate-100 pt-3 sm:grid-cols-2 md:grid-cols-4">
                <div className="flex flex-col gap-1.5">
                  <label className="text-xs font-semibold uppercase tracking-wider text-slate-400">Branch Node</label>
                  <select
                    value={filters.branch}
                    onChange={(e) => update({ branch: e.target.value === "all" ? "all" : Number(e.target.value) })}
                  ...
                  >
                    ...
                    {branches.map((branch) => {
                      const id = branch.b_id ?? branch.B_id ?? branch.bId;
                      const name = branch.B_name ?? branch.b_name ?? branch.name ?? `Branch #${id}`;
                      return (
