...
export async function createPurchaseItem(req, res, next) {
  try {
    ...
    if (parsedUnitPrice > MAX_UNIT_PRICE) {
      res.status(400);
      throw new Error(
        `unit_price cannot exceed ${MAX_UNIT_PRICE.toLocaleString()}`,
      );
    }
...
export async function updatePurchaseItem(req, res, next) {
  try {
    ...
    // ── Numeric validations ──
    const parsedQty =
      qty !== undefined ? parsePositiveDecimal(qty, "qty") : null;
    const parsedUnitPrice =
      unit_price !== undefined
        ? parsePositiveDecimal(unit_price, "unit_price")
        : null;
    const parsedPrice =
      price !== undefined ? parsePositiveDecimal(price, "price") : null;
