...
export async function createRecipeBulk(req, res, next) {
  ...
  try {
    ...
    // ── Verify each raw material belongs to user's company and get their names ──
    const rmIdsList = ingredients.map((i) => i.rawmaterial_id);
    let rmCheckQuery = 'SELECT "rm_id", "rm_name", "unit", "Com_id" FROM "public"."Raw_Material" WHERE "rm_id" = ANY($1)';
    const rmCheckParams = [rmIdsList];
    if (req.user.role_id !== ROLES.SUPER_ADMIN) {
      rmCheckQuery += ` AND "Com_id" = $2`;
      rmCheckParams.push(req.user.com_id);
    }
