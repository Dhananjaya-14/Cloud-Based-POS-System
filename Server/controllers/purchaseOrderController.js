...
export async function createPurchaseOrder(req, res, next) {
  try {
    ...
    if (received_date && resolvedStatus !== "received") {
      res.status(400);
      throw new Error(
        "received_date can only be set when status is 'received'",
      );
    }
...
export async function updatePurchaseOrder(req, res, next) {
  try {
    ...
    if (currentStatus === "received" && status && status !== "received") {
      res.status(409);
      throw new Error(
        "Cannot revert a received purchase order back to pending",
      );
    }
...
export async function deletePurchaseOrder(req, res, next) {
  try {
    ...
    if (err?.code === "23503") {
      ...
      return next(
        new Error(
          "Cannot delete purchase order because it is referenced elsewhere",
        ),
      );
    }
    next(err);
